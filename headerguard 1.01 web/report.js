...
// ---------- Análisis de contenido (API IA) ----------
function renderApi(r) {
  const card = document.getElementById("apiCard");
  if (!card) return;

  const data = r && r.contentRaw;
  if (!data || !data.resultado) {
    // Si no hay datos de la API, escondemos la tarjeta
  ...
  const summaryEl = document.getElementById("apiSummary");
  const detailsEl = document.getElementById("apiDetails");

  const res = data.resultado || {};
  const heur = data.analisis_heuristico || {};
  const ia = data.analisis_ia || {};

  const verdict = res.veredicto || "—";
  const prob = res.probabilidad_phishing || "—";
  const alertsNum = (typeof res.alertas_detectadas === "number")
    ? res.alertas_detectadas
    : (res.alertas_detectadas || 0);

  const hasAlerts = alertsNum > 0 ||
    (Array.isArray(heur.alertas) && heur.alertas.length > 0);

  // Resumen principal
  summaryEl.innerHTML = `
    <p><strong>Veredicto de la IA:</strong> ${verdict}</p>
    <p><strong>Probabilidad de phishing:</strong> ${prob}</p>
    <p><strong>Alertas detectadas:</strong> ${alertsNum}</p>
    ${hasAlerts
      ? `<p style="color:#DC2626;font-weight:bold">⚠ Se detectaron alertas en el contenido.</p>`
      : `<p style="color:#16A34A;">Sin alertas reportadas por la IA.</p>`}
  `;

  // URLs analizadas
  let urlsHtml = "";
  const urls = heur.detalle_urls || [];
  if (urls.length) {
    urlsHtml += `<h4>URLs analizadas</h4><ul>`;
    urls.forEach(u => {
      const urlDisplay = (u.url_analizada || u.url || "").replace(/</g,"&lt;");
      urlsHtml += `<li>
        <code>${urlDisplay}</code>
        — riesgo IA: ${u.riesgo_ia || "—"}
        ${u.es_acortador ? " (acortador)" : ""}
      </li>`;
    });
    urlsHtml += `</ul>`;
  }

  // ⚠ Alertas heurísticas (como las de tu ejemplo)
  let alertsHtml = "";
  if (Array.isArray(heur.alertas) && heur.alertas.length) {
    alertsHtml += `<h4>Alertas heurísticas</h4><ul>`;
    heur.alertas.forEach(a => {
      alertsHtml += `<li>${a}</li>`;
    });
    alertsHtml += `</ul>`;
  }

  // Scores de modelos IA
  let modelsHtml = "";
  if (ia && (ia.modelo_multilingue || ia.modelo_espanol || ia.modelo_urls)) {
    modelsHtml += `<h4>Modelos de IA</h4><ul>`;
    if (ia.modelo_multilingue) {
      modelsHtml += `<li>Modelo multilingüe: ${ia.modelo_multilingue}</li>`;
    }
    if (ia.modelo_espanol) {
      modelsHtml += `<li>Modelo español: ${ia.modelo_espanol}</li>`;
    }
    if (ia.modelo_urls) {
      modelsHtml += `<li>Modelo URLs: ${ia.modelo_urls}</li>`;
    }
    modelsHtml += `</ul>`;
  }

  const combinedHtml = urlsHtml + alertsHtml + modelsHtml;

  detailsEl.innerHTML = combinedHtml || "<p>No se recibieron detalles adicionales de la API.</p>";
}
