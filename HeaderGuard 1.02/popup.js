...
// popup.js - Modificar la función fill para mostrar análisis del cuerpo
function fill(report){
  ...
  if(!checks.length){ 
    const li=document.createElement("li"); li.className="empty"; li.textContent="Sin señales disponibles."; ul.appendChild(li); 
  } else {
    checks.slice(0,8).forEach(c=>{ 
      const li=document.createElement("li"); 
      li.textContent = `${icon(c.status)} ${c.label} — ${c.details||""}`; 
      ul.appendChild(li); 
    });
    
    // Mostrar información del análisis del cuerpo si existe
    if (bodyAnalysis) {
      const li = document.createElement("li");
      li.innerHTML = `🤖 <strong>Análisis ML:</strong> ${bodyAnalysis.resultado.veredicto} (${bodyAnalysis.resultado.probabilidad_phishing})`;
      ul.appendChild(li);
    }
    
    if(checks.length>8){ 
      const li=document.createElement("li"); li.textContent = `(+${checks.length-8} más...)`; ul.appendChild(li); 
    }
  }
}
